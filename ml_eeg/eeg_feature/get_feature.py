# -*- coding: utf-8 -*-
"""
Created on Wed Dec 28 11:11:52 2016
...
#%%
def psd_mne_multitaper(epoch_data,locutoff,hicutoff):
    from mne.time_frequency import psd_multitaper
    
    fmin = locutoff
    fmax = hicutoff
    psds, freqs = psd_multitaper(epoch_data, fmin=fmin, fmax=40, n_jobs=fmax)
    psds = 10 * np.log10(psds)  # dB

    return psds, freqs    
    
#%%
def psd_percent_multitaper(epoch_data,locutoff,hicutoff):
    psds, freqs = psd_mne_multitaper(epoch_data,locutoff,hicutoff)
...
def feature_set(ML):
    ...
    clean_label = pickle.load(open(filename_label, 'rb'))    
    
    locutoff = ML['locutoff']
    hicutoff = ML['hicutoff']
    
    if ML['FeatureType'] == 1:
        psds, freqs = psd_mne_multitaper(clearn_data,locutoff,hicutoff)        
        feature_data = psds[:,0]
        
#    elif ML['FeatureType'] == 2:
#        feature_data_label = psd_percent(clearn_data,locutoff,hicutoff)
...
#        feature_data_label = combined_feature(ML,eeg_data_label)
    ...
    # save 
    filename_data = ML['Feature_File'] + '_feature_data.csv'
    pickle.dump(feature_data, open(filename_data, 'wb'))
    filename_label = ML['Feature_File'] + '_feature_label.csv'
    pickle.dump(feature_label, open(filename_label, 'wb'))    
